foam.CLASS({
  package: 'net.codemirror',
  name: 'Editor',
  extends: 'foam.u2.tag.TextArea',
  documentation: `
    A code editor taken from https://codemirror.net
    TODO: We should find a better story around loading things like this from third parties.
    Currently, the JS and CSS was just copied from codemirror.net and pasted here.
  `,
  constants: [
    {
      name: 'CodeMirror',
  ...
  `,
  reactions: [
    ['', 'propertyChange.data', 'updateEditorFromData'],
    ['', 'onload', 'loadEditor']
  ],
  properties: [
    'editor'
  ...
  listeners: [
    function loadEditor() {
      ...
      this.editor = this.CODE_MIRROR.fromTextArea(this.el(), {
        lineNumbers: true,
        mode: "javascript"
      });
      this.editor.on('change', this.updateDataFromEditor);
      this.updateEditorFromData();
    },
    {
      name: 'updateEditorFromData',
      code: function() {
        if ( ! this.editor ) return;
        if ( this.editor.getValue() == this.data ) return;
        this.editor.setValue(this.data);
      }
    },
    {
      name: 'updateDataFromEditor',
      isFramed: true,
      code: function() {
