...
`((FixedThreadPool) getThreadPool()).submit(getX(), new ContextAgent() {
  ...
  public void execute(X x) {
    ...
    },
    {
      ...
      javaCode:
`DAOResourceLoader daoResourceLoader = new DAOResourceLoader;
EmailTemplate emailTemplate = daoResourceLoader.load(name);
...
if ( config == null ) {
  config = EnvironmentConfigurationBuilder
      ...
      .resources()
      .resourceLoaders()
      .add(new TypedResourceLoader("dao", emailTemplate))
      .and().and()
      .build();
  setConfig(config);
}
