  ...
  foam.CLASS({
    ...
    methods: [
      function initE() {
        ...
        this.NotificationView.create({of: this.data.cls_})
        /* if ( this.data.hasOwnProperty('linkObj') ) {
          this.start(this.LINK, { label$: this.data.linkLabel$ }).end()
        }
        if ( this.data.hasOwnProperty('popupObj') ) {
          this.start(this.POPUP, { label$: this.data.popupLabel$ }).end()
        } */
      },

      function detailE() {
        return this.E('h3').
          add(this.selection.id$, ' details!').
          br().
          add(this.SELECTION, this.EDIT);
      }
    ],
    ...
    actions: [
      /* {
        ...
        label: '',
        code: function() {
          var linkObj = this.data.linkObj;
          linkObj.class = linkObj.view;
          this.stack.push(linkObj);
        }
      },
      {
        name: 'popup',
        label: '',
        code: function() {
          this.add(this.Popup.create().tag(this.data.popupObj));
        }
      }, */
      {
        ...
        code: function (X) {
          ...
          self.optionPopup_.addClass('popUpDropDown')
            ...
            .end()
            //if ( this.data.notificationType != 'General' ) {
