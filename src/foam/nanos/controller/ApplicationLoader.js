foam.CLASS({
  ...
  properties: [
    ...
    },
    {
      ...
      factory: function() {
        var self = this;
        return self.classloader.load(self.controller).then(function(cls) {
          return new Promise(function(resolve) {

            var client = {
              refines: cls.model_.id,
              requires: [
                'foam.dao.EasyDAO',
              ],
              exports: [],
            ...
            self.nSpecDAO.where(self.EQ(self.NSpec.SERVE, true)).select({
              ...
              eof: function() {
                Promise.all(references).then(function() {
                  ...
                  var args = {};
                  self.ApplicationConfig.getAxiomsByClass(foam.core.Property).forEach(function(p) {
                    args[p.name] = self[p.name]
                  })
                  resolve(cls.create(args));
                });
              }
            });
          })
