...
/**
 * A Copy of BenchmarkRunner with the only notable change
 * being to capture failures.
 */
  ...
  implements ContextAgent
{
  ...
  protected ConcurrencyTest test_;
  protected String    result_ = "";
  final protected Map results_ = new HashMap();
  ...
  public static class Builder<T extends Builder<T>>
    extends ContextAwareSupport
  {
    protected String    name_            = "foam.nanos.test.ConcurrencyTestRunner";
    ...
    public T setConcurrencyTest(ConcurrencyTest val) {
      test_ = val;
      return (T) this;
    }

    public ConcurrencyTestRunner build() {
      return new ConcurrencyTestRunner(getX(),this);
    }
  }

  protected ConcurrencyTestRunner(X x, Builder<?> builder) {
    setX(x);
    name_            = builder.name_;
  ...
  public void execute(final X x) {
    try {
        ...
        Thread thread = new Thread(group, new Runnable() {
          ...
          public void run() {
            for (int j = 0; j < invocationCount_; j++) {
              test_.execute(x);
              String s = test_.result();
              if ( s != null ) {
                //results_.put(Thread.currentThread().getName()+"-"+tno+"_"+j, s);
                results_.put(Thread.currentThread().toString()+"_"+j, s);
              }
            }
      ...
      result_ =
        ...
        "Failures: " + results_.size() + "\n";

      ((Logger) x.get("logger")).info(result_);
