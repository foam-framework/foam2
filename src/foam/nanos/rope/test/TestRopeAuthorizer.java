    ...
    for ( ROPE rope : ropes ) {
      ...
      if ( ( rope.getRelationshipImplies()).contains(operation) && sourceObjs.size() > 0 ) {
        for ( FObject sourceObj : sourceObjs ) {
          if ( ( sourceObj instanceof ROPEUser && obj instanceof ROPEUser ) ) return true;
      ...
      if ( actions != null && actions.size() > 0 ) {
        for ( FObject sourceObj : sourceObjs ) {
          for ( ROPEActions action : actions ) {
            if ( ( sourceObj instanceof ROPEUser && obj instanceof ROPEUser ) || ropeSearch(action, sourceObj, x, rope.getSourceDAOKey()) ) return true;
          }
        }
      }
    }
