...
public class QuorumNetworkManager extends AbstractFObject {
	...
	ConcurrentHashMap<Long, ArrayBlockingQueue<QuorumMessage>> intanceToQueueMap;
	...
	public class Sender extends FoamThread {
		...
		public void run() {
			try {
				while ( isRunning ) {
					try {
						ArrayBlockingQueue<QuorumMessage> messageQueue = intanceToQueueMap.get(instanceId);
	...
	public void sendToInstace(Long instanceId, QuorumMessage message) {
		if ( quorumName.getId() == instanceId ) {
			appendToReceiveQueue(message);
			return;
		}
		
		ArrayBlockingQueue<QuorumMessage> queue = intanceToQueueMap.get(instanceId);
		if ( queue == null ) {
			queue = intanceToQueueMap.put(instanceId, new ArrayBlockingQueue<QuorumMessage>(1));
		}
	...
	public synchronized boolean initialConnection(Socket socket, Long instanceId, InetSocketAddress addr) {
		...
		intanceToQueueMap.putIfAbsent(instanceId, new ArrayBlockingQueue<QuorumMessage>(1));
	...
	public void addConnection(Socket socket) {
		...
		if ( message.getSourceInstance() < quorumName.getId() ) {
			...
			intanceToQueueMap.putIfAbsent(message.getSourceInstance(), new ArrayBlockingQueue<QuorumMessage>(1));
