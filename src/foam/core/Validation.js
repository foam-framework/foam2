    ...
    },
    {
      ...
      expression: function(name, label, required, validationPredicates) {
        ...
        return !required ? null : [[name],
          function() {
            return !this.hasOwnProperty(name) && (`Please enter a ${label.toLowerCase()}`);
          }]
      },
    },
  ]
      ...
      factory: function() {
        ...
        if ( foam.Number.isInstance(this.minLength) ) {
          a.push({
            ...
            errorString: `Please enter a ${this.label.toLowerCase()} with least ${this.minLength} character${this.minLength>1?'s':''}`
          });
        }
        if ( foam.Number.isInstance(this.maxLength) ) {
          a.push({
            ...
            errorString: `Please enter a ${this.label.toLowerCase()} with at most ${this.maxLength} character${this.maxLength>1?'s':''}`
          });
        }
        return a;
      }
      ...
      expression: function(name, label, required, validationPredicates, autoValidate) {
        if ( autoValidate ) {
          return [
            [`${name}$errors_`],
            function(errs) {
              return errs ? `Please enter a valid ${label.toLowerCase()}` : null;
            }
          ];
        }
      ...
      factory: function() {
        ...
        if ( foam.Number.isInstance(self.min) ) {
          a.push({
            ...
            errorString: `Please enter a ${self.label.toLowerCase()} greater than or equal to ${self.min}.`
          });
        }
        if ( foam.Number.isInstance(self.max) ) {
          a.push({
            ...
            errorString: `Please enter a ${self.label.toLowerCase()} less than or equal to ${self.max}`
          });
        }
        return a;
      }
