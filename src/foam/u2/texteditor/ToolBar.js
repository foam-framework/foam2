/**
 * @license
 * Copyright 2019 The FOAM Authors. All Rights Reserved.
 * http://www.apache.org/licenses/LICENSE-2.0
 */

foam.CLASS({
  package: 'foam.u2.texteditor',
  name: 'ToolBar',
  extends: 'foam.u2.View',

  requires: [
    'foam.u2.texteditor.DefaultTextFormats',
    'foam.u2.texteditor.FontFace',
    'foam.u2.texteditor.FontSize',
    'foam.u2.texteditor.TextAlignment',
    'foam.u2.texteditor.TextFormats',
    'foam.u2.texteditor.ListStyle',
    'foam.u2.texteditor.Popup',
    'foam.u2.DetailView',
    'foam.u2.texteditor.ToolBarItemView',
    'foam.u2.tag.Image'
  ],

  css: `
    ^richtext-actions {
      -webkit-align-items: center;
      align-items: center;
      border: none;
      -webkit-border-radius: 2px;
      border-radius: 2px;
      -webkit-box-shadow: 0 4px 5px 0 rgba(0,0,0,0.14), 0 1px 10px 0 rgba(0,0,0,0.12), 0 2px 4px -1px rgba(0,0,0,0.2);
      box-shadow: 0 4px 5px 0 rgba(0,0,0,0.14), 0 1px 10px 0 rgba(0,0,0,0.12), 0 2px 4px -1px rgba(0,0,0,0.2);
      display: -webkit-inline-box;
      display: -webkit-inline-flex;
      display: -ms-inline-flexbox;
      display: inline-flex;
      margin: 16px;
      padding: 8px 2px;
      white-space: nowrap;

      /* Firefox */
      display: -moz-box;
      -moz-box-pack: center;
      -moz-box-align: center;

      /* Safari and Chrome */
      display: -webkit-box;
      -webkit-box-pack: center;
      -webkit-box-align: center;

      /* W3C */
      display: box;
      box-pack: center;
      box-align: center;
    }

    ^richtext-actions button {
      width: auto;
      height: 40px;
      text-align: center;
      font-size: 14px;
      background: none !important;
      color: #373a3c !important;
      border: none;
      font-weight: normal;
      border-radius: 0px;
  
      -webkit-box-shadow: inset 0 0 0 0 #ffffff;
    }
  
    ^richtext-actions button:focus {
      outline: none;
      background: #ddd !important;
    }

   ^toolbar-item {
     width: auto;
     height: auto;
     text-align: center;
     font-size: 14px;
     background: none !important;
     color: #373a3c !important;
     padding: 4px 12px;
   }  
 `,

 exports: [
  'doc_ as document'
],

  properties: [
    {
      name: 'doc_',
      factory: function() {
        return this.document;
      }
    }
  ],

  listeners: [
    {
      name: 'refresh',
      mergeDelay: 300,
      isMerged: true,
      code: function() {
        this.doc_ = null;
        this.doc_ = this.document;
        this.refresh();
      }
    }
  ],

  methods: [
    function initE() {
      this.refresh();

      this.SUPER();
      this.addClass(this.myClass('richtext-actions'));
      this
        .start(this.DetailView, { data: this.DefaultTextFormats.create(), showActions: true, title: ''})
          .addClass(this.myClass('toolbar-item'))
        .end()
        .start(this.Popup, {
          button: this.Image.create({ data: this.FONT_SIZE_BUTTON}),
          view: { class: 'foam.u2.DetailView', data: this.FontSize.create(), showActions: true, title: ''}
        })
          .addClass(this.myClass('toolbar-item'))
        .end()
        .start(this.Popup, {
          button: this.Image.create({ data: this.FONT_FACE_BUTTON}),
          view: { class: 'foam.u2.DetailView', data: this.FontFace.create(), showActions: true, title: ''}
        })
          .addClass(this.myClass('toolbar-item'))
        .end()        
        .start(this.Popup, {
          button: this.Image.create({ data: this.TEXT_ALIGNMENT_BUTTON}),
          view: { class: 'foam.u2.DetailView', data: this.TextAlignment.create(), showActions: true, title: ''}
        })
          .addClass(this.myClass('toolbar-item'))
        .end()
        .start(this.DetailView, { data: this.ListStyle.create(), showActions: true, title: ''})
          .addClass(this.myClass('toolbar-item'))
        .end()
        .start(this.Popup, {
          button: this.Image.create({ data: this.TEXT_FORMAT_BUTTON}),
          view: { class: 'foam.u2.DetailView', data: this.TextFormats.create(), showActions: true, title: ''}
        })
          .addClass(this.myClass('toolbar-item'))
        .end();
    },
  ],
  constants: {
    FONT_SIZE_BUTTON: 'data:image/png;base64,',
    FONT_FACE_BUTTON: 'data:image/png;base64,',
    TEXT_ALIGNMENT_BUTTON: 'data:image/svg+xml;base64,',
    TEXT_FORMAT_BUTTON: 'data:image/png;base64,'
  },
});
