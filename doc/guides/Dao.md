...
mLangs are composable objects that allow queries to be assembled without knowing
the underlying target query language. mLangs serialize easily, or compile
themselves into SQL, server-specific query formats, or run directly on an
indexed, in-memory `foam.dao.MDAO`.
...
If the sink has an unrecoverable error, for instance, calling `errorEvt()`
on the flow control object will hint that the upstream DAO should stop sending
`put()`s. If an internal limit is reached, a call to `stop()` will halt events
but not indicate that an error occurred.
...
a collection of results. If unfiltered, `select(sink)` returns everything in
the DAO.

    dao.select().then(function(sink) {
      console.log("Default sink with the entire DAO contents:", sink.a);
    });
...
see when you `find`, `select`, and `removeAll`. You can call these methods
