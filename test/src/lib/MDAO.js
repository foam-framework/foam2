...
function atime(name, promise) {
  ...
  var fn = function(arg) {
    ...
    console.log("Time for ",name,": ",startTime - endTime);
...
aseq(
  ...
  arepeat(DEBUG ? 1 : 7, aseq(
    ...
    atest('2cMultiKeyQuery10',      function(ret) { PhotoDAO.where(M.IN(Photo.ID, KEYS_10)).select()(ret); }),
    aif(!DEBUG, aseq(
    atest('2cMultiKeyQuery100',     function(ret) { PhotoDAO.where(M.IN(Photo.ID, KEYS_100)).select()(ret); }),
    atest('2cMultiKeyQuery1000',    function(ret) { PhotoDAO.where(M.IN(Photo.ID, KEYS_1000)).select()(ret); }),
    atest('2cMultiKeyQuery5000',    function(ret) { PhotoDAO.where(M.IN(Photo.ID, KEYS_5000)).select()(ret); })
    )),
    atest('2dIndexedFieldQuery',    function(ret) {
      PhotoDAO.where(M.EQ(Photo.ALBUM_ID, avgKey)).select(MAP(Photo.ALBUM_ID))(ret);
    }),
    atest('2dIndexedFieldQuery(X10)', arepeat(10,function(ret) {
      PhotoDAO.where(M.EQ(Photo.ALBUM_ID, avgKey)).select(MAP(Photo.ALBUM_ID))(ret);
    })),
    atest('2eAdHocFieldQuery',      function(ret) {
      PhotoDAO.where(M.EQ(Photo.IS_LOCAL, true)).select(MAP(Photo.HASH))(ret);
    }),
    atest('2fSimpleInnerJoinQuery', function(ret) {
      AlbumDAO.where(M.EQ(Album.IS_LOCAL, false)).select(MAP(Album.ID))(function (ids) {
        PhotoDAO.where(M.IN(Photo.ALBUM_ID, ids.arg2)).select()(ret);
    })}),
  //  atest('2fSimpleInnerJoinQuery(Simpler+Slower Version)', function(ret) {
  //    AlbumDAO.where(M.EQ(Album.IS_LOCAL, true)).select(MAP(JOM.IN(PhotoDAO, Photo.ALBUM_ID, []), []))(ret);
  //  }),
...
//     })}),
  ...
  //        MAP(JOM.IN(PhotoDAO, Photo.ALBUM_ID, SUM_PHOTO_COUNT), []))(ret);
  //  }),
    atest('2hSimpleOrderByQuery', function(ret) {
      PhotoDAO.where(M.EQ(Photo.ALBUM_ID, avgAlbumKey)).orderBy(M.DESC(Photo.TIMESTAMP)).select()(ret);
    }),
...
//     }),
//    atest('2jSimpleAggregationQuery', function(ret) { PhotoDAO.select(M.GROUP_BY(Photo.ALBUM_ID))(ret); }),
  /*
